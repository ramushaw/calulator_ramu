...
# ============================================================
    ...
    degree = st.number_input("Enter angle in degrees:", value=0.0)
    ...
    if st.button("Calculate", key="trigonometry_button"):
        ...
        if function == "sin":
            ...
            if math.isclose(math.tan(radian), 0, abs_tol=1e-10):
            ...
            if math.isclose(math.cos(radian), 0, abs_tol=1e-10):
            ...
            if math.isclose(math.sin(radian), 0, abs_tol=1e-10):
...
# ============================================================
    ...
    if machining_option == "EN Steel Cutting-Speed Information":
        st.write("Reference data for selected EN steels.")
        steel = st.selectbox("Choose EN steel:", list(EN_STEELS.keys()))
        data = EN_STEELS[steel]

        st.subheader(f"{steel} — Reference Information")
        st.write(f"Average carbon content: **{data['carbon']:.2f}%**")
        st.write(f"Recommended CS with uncoated carbide: **{data['uncoated_CS']} m/min**")
        st.write(f"Recommended CS with coated carbide / insert: **{data['coated_CS']} m/min**")
    ...
    # ========================================================
        ...
        choice = st.selectbox(
            ...
            ["Cutting Speed (CS)", "RPM", "Diameter"]
        )

        # --- CUTTING SPEED ---
        if choice == "Cutting Speed (CS)":
            diameter = st.number_input("Enter diameter in mm:", min_value=0.0, value=0.0)
            rpm = st.number_input("Enter RPM:", min_value=0.0, value=0.0)

            if st.button("Calculate Cutting Speed", key="cs_button"):
        ...
        # --- RPM ---
            ...
            cs = st.number_input("Enter cutting speed in m/min:", min_value=0.0, value=0.0)
            diameter = st.number_input("Enter diameter in mm:", min_value=0.0, value=0.0)

            if st.button("Calculate RPM", key="rpm_button"):
        ...
        # --- DIAMETER ---
            ...
            if st.button("Calculate Diameter", key="diameter_button"):
                if cs <= 0 or rpm <= 0:
                    st.error("Please enter values greater than zero.")
                else:
